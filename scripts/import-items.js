...
// scripts/localize-item-names.js. The BeastBurst API still hands back raw
// keys for many items; we resolve them here at import time so Sanity always
// holds the player-facing name.
let LOCAL_NAME_MAP = {};
try {
  LOCAL_NAME_MAP = require('../src/lib/itemLocalizationMap.json');
} catch { /* manifest not generated yet — leave names untouched */ }
function localizeName(name) {
  if (typeof name !== 'string') return name;
  if (!/Key$/.test(name) || /\s/.test(name)) return name;
  return LOCAL_NAME_MAP[name] || name;
}
...
async function main() {
  ...
  const slugMap = assignSlugs(
    allItems.map((i) => ({ id: i.id, name: localizeName(i.name) })),
    existing,
  );
