  ...
  try {
    const build = await sanityClient.fetch(
      `*[_type == "talentBuild" && code == $code][0]{
        code, classSlug, allocation, equipment, name, tags, description, guide, patch, totalPoints, upvotes, downvotes, createdAt, isPublic, publishedAt, "authorName": author->displayName, "authorImage": author->image, "authorRef": author._ref
      }`,
      { code },
    );
