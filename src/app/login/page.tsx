...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (session) router.replace('/builds');
  }, [session, router]);
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <h1 className="font-heading text-3xl text-honor-gold mb-3">Sign In</h1>
          <p className="text-text-secondary text-sm">
            Sign in to save builds, share with the community, and track your theorycraft.
          </p>
        </div>

        <div className="bg-card-bg border border-border-subtle rounded-lg p-6 space-y-4">
          <button
            onClick={() => signIn('twitch', { callbackUrl: '/builds' })}
          ...
          <button
            onClick={() => signIn('discord', { callbackUrl: '/builds' })}
          ...
          <button
            onClick={() => signIn('google', { callbackUrl: '/builds' })}
        ...
        <p className="text-center text-xs text-text-muted mt-6">
          No password needed. We only use your name and avatar.
        </p>
      </div>
    </div>
  );
